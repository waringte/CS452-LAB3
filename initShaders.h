...
GLuint initShaders(ShaderData* shaders);
...
GLuint initShaders(ShaderData* shaders){
  ...
  ShaderData* shade=shaders;
  
  vector<GLuint> shadeList;
  
  while(shade->type != GL_NONE){
    shadeList.push_back(createShader(shade->type,read(shade->file)));
    ++shade;
  }
  
  GLuint program=createShaderProgram(shadeList);
